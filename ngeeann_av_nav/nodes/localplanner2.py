...
class LocalPathPlanner:
    ...
    def determine_path(self, cx, cy, cyaw):
        ...
        if len(collisions) != 0 and (current_target - self.react_dist) <= collisions[-1]:
    ...
    def collision_reroute(self, cx, cy, cyaw, ax, ay, dev_id, intersect_id):
        ...
        if (len(ax) >= 4):
            try:
                avoid_cx, avoid_cy, avoid_cyaw, _, _ = calc_spline_course(ax[1:-2], ay[1:-2], self.ds)
            except:
                # Draws straight line when spline unavailable to insufficient points
                avoid_cx = [ax[1], ax[-2]]
                avoid_cy = [ay[1], ay[-2]]
                avoid_cyaw = [np.arctan2((ay[-2]-ay[1]), (ax[-2]-ax[1]))]
        else:
            # Draws straight line when spline unavailable to insufficient points
            avoid_cx = [ax[1]]
            avoid_cy = [ay[1]]
